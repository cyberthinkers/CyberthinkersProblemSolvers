...
abstract class AbstractMutableVariable extends AbstractVariable {
  var value: Double
  def approxEqual(thatValue: AbstractMutableVariable) = Math.abs(this.value - thatValue.value) < AbstractVariable.epsilon
  def isApproxZero: Boolean = Math.abs(this.value) < AbstractVariable.epsilon
 }
